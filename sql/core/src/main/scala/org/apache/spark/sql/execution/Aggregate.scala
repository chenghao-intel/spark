...
case class AggregatePreShuffle(
   ...
   */
  private[this] def createIterator(iterator: Iterator[(Row, MutableRow)], functions: Seq[AggregateFunction]) =
    new Iterator[Row] {
      ...
      override final def next(): Row = {
        ...
        while (idx < functions.length) {
          ...
          af.terminatePartial(currentEntry._2)
          idx += 1
        }
  ...
  override def execute() = attachTree(this, "execute") {
    child.execute().mapPartitions { iter =>
      ...
      createIterator(results.iterator, functions)
    }
  }
}
