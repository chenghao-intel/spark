...
class PlannerSuite extends FunSuite {
  ...
  test("count is partially aggregated") {
    val query = testData.groupBy('value)(Count('key)).queryExecution.sparkPlan
  ...
  test("count distinct is partially aggregated") {
    val query = testData.groupBy('value)(CountDistinct('key :: Nil)).queryExecution.sparkPlan
  ...
  test("mixed aggregates are partially aggregated") {
    val query =
      testData.groupBy('value)(Count('value), CountDistinct('key :: Nil)).queryExecution.sparkPlan
    // TODO currently only reducer side aggregation support for DISTINCT
    val shuffles = query.collect { case n: DistinctAggregate => n }
    assert(shuffles.size === 1)
  }
